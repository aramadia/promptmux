...
import { waitForElement, injectTextIntoElement, triggerSubmit } from './dom-utils';
...
contextBridge.exposeInMainWorld('promptMux', {
  injectPrompt: async (text: string): Promise<boolean> => {
    ...
    try {
      ...
      const injected = injectTextIntoElement(input, text, selectors.isContentEditable);
      if (!injected) {
      ...
      if (!submitted) {
        console.warn('[PromptMux][ChatGPT] Could not find submit button, trying Enter key');
        input.dispatchEvent(
